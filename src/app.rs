...
// -----------------------------------------------------------------------------
// メインのappコンポーネント (命名規則を小文字に強制)
// -----------------------------------------------------------------------------
#[component]
pub fn app() -> impl IntoView {
    let (messages, set_messages) = signal(vec![Message {
        is_user: false,
        text: "こんにちは！何かお手伝いできることはありますか？".to_string(),
    }]);

    let (input_text, set_input_text) = signal(String::new());

    // Wasm (シングルスレッド) 用に Action::new_local を採用し、Send 制約を回避
    let send_message = Action::new_local(move |input: &String| {
